...
const queryList = async (pageCurrent, pageSize) => {
  try {
    let { data, errCode } = await addressCloudObj.list({
      pageCurrent,
      pageSize,
    });
...
const defaultChange = async (e, item) => {
  try {
    ...
    let { errCode, data } = await addressCloudObj.update({
      _id: item._id,
    ...
    // 更新pinia状态
    if (isDefault) {
      orderStore.addressInfo = { ...item, is_default: true };
    } else if (orderStore.addressInfo?._id === item._id) {
      orderStore.addressInfo.is_default = false;
    }
...
<template>
  <view class="page-wrap">
    <z-paging ref="paging" v-model="dataList" @query="queryList">
      ...
      <view class="list-wrap">
        ...
        >
          ...
          <view class="info">
            ...
            <view
              class="edit"
              @click.stop="routerTo(`/pages/my/addressItem?id=${item._id}`)"
            >
      ...
      <template #bottom>
        <view class="addBtn">
          <button type="primary" @click="routerTo('/pages/my/addressItem')">
            + 新增收货地址
          </button>
