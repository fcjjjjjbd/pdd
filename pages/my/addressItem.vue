...
const pageTypeTxt = computed(() => (unref(id) ? "编辑" : "新增"));
onLoad((e) => {
  id.value = e.id;
  if (unref(id)) {
    getDetail();
...
const handleRemove = async () => {
  try {
    ...
    let { errCode, data } = await addressCloudObj.remove({
      _id: unref(formData)._id,
    });
    if (errCode !== 0) return showToast("删除失败");
    showToast("删除成功");
    uni.$emit("addressSuccess");
    uni.navigateBack();
  } catch (err) {
    showToast(err);
  }
...
const submitForm = async () => {
  try {
    ...
    showLoading({ mask: true });
      ...
      ? await addressCloudObj.update(unref(formData))
      : await addressCloudObj.add(unref(formData));
 ...
 // #ifdef H5
  ...
  let { errCode } = unref(formData)._id
      ? await addressCloudObj.update(unref(formData))
      : await addressCloudObj.addweb(unref(formData));

	// #endif

    if (errCode !== 0) return showToast(pageTypeTxt + "失败");
    showToast(pageTypeTxt + "成功");
    uni.$emit("addressSuccess");
    uni.navigateBack();
  } catch (err) {
    showToast(err);
...
const getDetail = async () => {
  try {
    let { data, errCode } = await addressCloudObj.detail({ _id: unref(id) });
    formData.value = { ...data, location: data.location?.coordinates || [] };
    areaValue.value =
      unref(formData).area[unref(formData).area.length - 1].value;
  } catch (err) {
    console.log(err);
  }
};
  ...
  <view class="page-wrap">
    <view class="form-wrap">
      ...
      >
        ...
        <view class="form-item-wrap">
          <uni-forms-item label="联系电话" name="phone" required>
            <view class="input-wrap">
              <uni-easyinput
                class="input disabled-input-click"
                placeholder="清输入联系电话"
        ...
        <view class="form-item-wrap">
          <uni-forms-item label="门牌号" name="house">
            <view class="input-wrap">
              <uni-easyinput
                class="input"
                placeholder="清输入门牌号（选填）"
                v-model.trim="formData.house"
              />
            </view>
          </uni-forms-item>
        </view>
      ...
      <view class="submit-wrap">
        ...
        <button class="btn submit" @click.stop="submitForm">
          {{ formData._id ? "确认修改" : "确认新增" }}
        </button>
      </view>
    </view>
  </view>
