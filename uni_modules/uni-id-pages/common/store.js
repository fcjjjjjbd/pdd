...
const uniIdTokenExpired = uni.getStorageSync("uni_id_token_expired") || 0;
const currentTimestamp = Date.now(); // 当前时间戳（秒）

const data = {
  userInfo: hostUserInfo,
  hasLogin:
    Object.keys(hostUserInfo).length != 0 &&
    uniIdTokenExpired > currentTimestamp,
};
...
export const mutations = {
  ...
  async updateUserInfo(data = false) {
    if (data) {
      ...
      try {
        let res = await usersTable
          .where("'_id' == $cloudEnv_uid")
          .field("mobile,nickname,username,email,avatar")
          .get();
