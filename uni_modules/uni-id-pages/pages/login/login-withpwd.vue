...
<template>
	<view class="uni-content">
		...
		<!-- 带选择框的隐私政策协议组件 -->
		<uni-id-pages-agreements scope="login" ref="agreements" v-if="false"></uni-id-pages-agreements>
		<button class="uni-btn" type="primary" @click="pwdLogin">永久登录</button>
		<view>新用户先注册,后登录</view>
		<view>如果一直登录不上,换浏览器登录</view>
		<!-- 忘记密码 -->
		<view class="link-box" >
			...
			<text class="link" @click="toRegister" v-if="!config.isAdmin ">账号</text>
		</view>
		</view>
		<!-- 悬浮登录方式组件 -->
...
// 	}
	...
	export default {
		...
		onShow() {
		 ...
		 mounted() {
    this.zhuce = this.checkIfRegistered();
  },
		methods: {
			   // 检查是否已注册
			checkIfRegistered() {
      return uni.getStorageSync("zhuce") || false;
    },
			// 页面跳转，找回密码
			toRetrievePwd() {
