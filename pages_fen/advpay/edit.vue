...
<template>
  <view class="home">
    <view class="phone">
      手机号
      <uv-input maxlength="11" v-model="dataobj.phone" type="number" shape="circle" placeholder="手机号"
        prefixIconStyle="font-size: 22px;color: #909399"></uv-input>
    </view>

    <!-- 添加图片 -->
    <view class="picarr">
      <view class="box add" @click="addpic" v-if="dataobj.imageValue.length < 1">
...
<script setup>
  ...
  const getDetail = async () => {
    let {
      errCode,
      data
    } = await pddyun.detailxg(unref(id));
    dataobj.value = data;
    console.log(data);
  };
